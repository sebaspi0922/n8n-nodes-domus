import { expect, test } from '@playwright/test';
import { hasDomusToken } from './helpers/env';
import {
	addDomusNode,
	createDomusCredentialViaApi,
	discoverPropertyCode,
	ensureOwner,
	executeOpenNode,
	expectDomusNodeOpen,
	openBlankWorkflow,
} from './helpers/n8n';

test.beforeEach(async ({ page }) => {
	await ensureOwner(page);
});

test('shows Property Code and Status on Change Status', async ({ page }) => {
	await openBlankWorkflow(page);
	await addDomusNode(page, 'Change property status');
	await expectDomusNodeOpen(page);

	await expect(page.getByText('Property Code', { exact: true })).toBeVisible();
	await expect(page.getByText('Status', { exact: true })).toBeVisible();
});

test('shows pagination controls on Get Status History', async ({ page }) => {
	await openBlankWorkflow(page);
	await addDomusNode(page, 'Get property status history');
	await expectDomusNodeOpen(page);

	await expect(page.getByText('Property Code', { exact: true })).toBeVisible();
	await expect(page.getByText(/return all/i).first()).toBeVisible();
});

test('executes Property Get Status History for a discovered testing property', async ({
	page,
}) => {
	test.skip(!hasDomusToken, 'Set DOMUS_TEST_TOKEN to execute history against Domus');

	const propertyCode = await discoverPropertyCode(page.request);
	test.skip(!propertyCode, 'Testing host returned no properties to inspect');

	await createDomusCredentialViaApi(page.request);
	await openBlankWorkflow(page);
	await addDomusNode(page, 'Get property status history');
	await expectDomusNodeOpen(page);

	const codeInput = page
		.getByTestId('parameter-input-propertyCode')
		.locator('input')
		.or(page.getByRole('textbox', { name: /property code/i }));
	await codeInput.first().fill(String(propertyCode));
	await executeOpenNode(page);

	await expect(page.getByText(/item|no output|success/i).first()).toBeVisible({
		timeout: 30_000,
	});
});
